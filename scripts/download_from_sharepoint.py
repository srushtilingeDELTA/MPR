...
def load_sharepoint_config() -> dict:
    with open(BASE_DIR / "config.yaml", "r", encoding="utf-8") as f:
        return (yaml.safe_load(f) or {}).get("sharepoint", {})


def main() -> int:
    parser = argparse.ArgumentParser(description="Download MPR Excel from SharePoint")
    parser.add_argument("--method", choices=["sdk", "selenium", "local"], default="selenium")
    ...
    try:
        ...
        print(
            ...
            "  3. In Edge: click the Excel file → Download manually\n"
            "  4. Save manually to data/ then run python main.py\n",
            file=sys.stderr,
        )
        return 1
